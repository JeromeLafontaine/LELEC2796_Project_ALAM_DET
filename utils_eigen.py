...
def eigen_decoder(y_eigen : np.ndarray , H : np.ndarray, u_max : np.ndarray):
    """ Eigen decoder
    Args:
    ...
    for i in range(len(y1)):
        ...
        r2 = y2[i]     # received symbol by antenna 2 at time ti

        umax = umax_array[:,i]

        z1[i] = umax[0] * r1 + umax[1]  * r2
        z2[i] = umax[0] * r1 + umax[1]  * r2
...
def eigen_Sionna(y_eigen : np.ndarray , h_eigen : np.ndarray, rg : sn.ofdm.ResourceGrid, batch_size : int, OFDM_pilots_time : list, u_max : np.ndarray, H : np.ndarray):
    """ eigen decoder
    Args:
    ...
    z = np.zeros((batch_size, num_tx, num_streams_per_tx, num_ofdm_symbols , 76), dtype = np.complex64)
    for i in range(batch_size):
        for j in range(num_ofdm_symbols):
            ...
            y_rx_2 = y_eigen_effective_subcarrier[i,1,0,j,:] #received symbol by antenna 2 at time time j

            y_temp = np.zeros((2,76), dtype = np.complex64)
            y_temp[0,:] = y_rx_1
            y_temp[1,:] = y_rx_2
            H_temp = H[j,:,:,:]   
            u_max_temp = u_max[j,:,:]
            z_temp = eigen_decoder(y_temp, H_temp , u_max_temp).numpy()
            z[i,:,0,j,:] = z_temp.T
...
def eigen_Sionna_ENCODER(y_eigen : np.ndarray , h_eigen : np.ndarray, 
                         rg : sn.ofdm.ResourceGrid, batch_size : int, 
                         OFDM_pilots_time : list):
    """ eigen decoder
    Args:
    ...
    for i in range(1):
        for j in range(num_ofdm_symbols):
            y_rx_1 = y_eigen[i,0,0,j,:]     # by antenna 1 at time time j
            y_rx_2 = y_eigen[i,1,0,j,:]     # by antenna 2 at time time j
            h_11 = h_eigen[i,0,0,0,0,j,:]   #h11 -> rx0 -> tx0
            h_12 = h_eigen[i,0,0,1,0,j,:]   #h12 -> rx0 -> tx1
            h_21 = h_eigen[i,1,0,0,0,j,:]   #h21 -> rx1 -> tx0
            h_22 = h_eigen[i,1,0,1,0,j,:]   #h22 -> rx1 -> tx1
            y_temp = np.zeros((2,y_eigen.shape[-1]), dtype = np.complex64)
            y_temp[0,:] = y_rx_1
            y_temp[1,:] = y_rx_2

            H_temp = np.array([[h_11,h_12],[h_21,h_22]])
            H[j,:,:,:] = H_temp
...
def eigen_BER_different_SNR(x_eigen_OFDM , SNR , batch_size, rg, OFDM_pilots_time, num_bits_per_symbol=2, num_ofdm_symbols=2, h_start = None, h_end = None, b = None):
  ...
  x_eigen_OFDM, u_max, H = eigen_Sionna_ENCODER(x_eigen_OFDM , h_eigen = h, rg =rg, batch_size = batch_size, OFDM_pilots_time = OFDM_pilots_time)

  # Application of the genated channel and recuperation of the received signal y
  y_eigen = apply_channel([x_eigen_OFDM, h ,no])

  y_hat = eigen_Sionna(y_eigen, h, rg, batch_size, OFDM_pilots_time , u_max , H)
